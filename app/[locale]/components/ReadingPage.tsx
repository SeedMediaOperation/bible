...
const ReadingPage = ({ versions, books, chapters, singleVersion }: ReadingProps) => {
    ...
    useEffect(() => {
        ...
        if (books.length > 0 && chapters.length > 0) {
            ...
            const firstBookChapters = chapters.filter(chap => chap.bookId === firstBook.id);

            setSelectedBook(firstBookChapters);
            setSelectedBookName(locale === "km" ? firstBook.nameKm ?? null : firstBook.nameEn ?? null);
            setSelectedBookId(firstBook.id);

            if (firstBookChapters.length > 0) {
                const chapterIndex = chapters.findIndex(chap => chap.id === firstBookChapters[0].id);
                setActiveChapterIndex(chapterIndex !== -1 ? chapterIndex : 0);
            }
        }
    }, [selectedVersionName, books, chapters, locale]);
    ...
    function khmerNumberToInt(khmerStr: string) {
        const khmerDigits = "០១២៣៤៥៦៧៨៩";
        let numStr = "";
        for (const ch of khmerStr) { // use const here
            const index = khmerDigits.indexOf(ch);
            if (index !== -1) numStr += index.toString();
            else numStr += ch; // keep other chars
        }
    ...
    return (
        <>
            <AlertContainer />
            <div className="w-full min-h-screen translate-y-0 px-2 mt-10">
                {/* Version Button */}
                <div className="sticky top-0 z-4">
                    ...
                    <div className="flex space-x-2 p-1 md:p-5">
                        {/* Book and Chapter */}
                        <button
                            onClick={() => setShowBookPopup(true)}
                            className="flex justify-between items-center w-full bg-[#000]/40 text-[#4FC9EE] rounded-full px-3 py-1 space-x-2"
                        >
                            <span className={`text-[14px] md:text-[16px] ${locale === 'km' ? 'font-krasar' : 'font-gotham'}`}>
                        ...
                        <button
                            ...
                            <span className={`text-[14px] md:text-[16px] ${locale === 'km' ? 'font-krasar' : 'font-gotham'}`}>{locale === 'km' ? 'ផ្លាស់ប្តូរជំពូក' : 'Change Chapters'}</span>
                            <span>
                                <svg width="12" height="12" viewBox="0 0 16 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M9.73205 12C8.96225 13.3333 7.03775 13.3333 6.26795 12L1.0718 3C0.301995 1.66667 1.26424 1.46309e-06 2.80384 1.32849e-06L13.1961 4.19966e-07C14.7358 2.8537e-07 15.698 1.66667 14.9282 3L9.73205 12Z" fill="#00AFD7" />
                                </svg>
                                        ...
                                        <SwiperSlide key={chapterIndex}>
                                            ...
                                            {(locale === 'km' ? p.paragraphKm : p.paragraphEn).map((line, index) => {
                                                ...
                                                const handleClick = () => {
                                                    ...
                                                    setSelectedBookName(matchedBookName ?? '');
                                                    setActiveChapterIndex(chapterIndex);
                            ...
                            <div className={`w-full inline-flex justify-center`}>
                                ...
                                >
                                    {chapters
                                        ...
                                        .sort((a, b) =>
                                            locale === 'km'
                                            ? khmerNumberToInt(a.nameKm || "") - khmerNumberToInt(b.nameKm || "")
                                            : Number(a.nameEn || 0) - Number(b.nameEn || 0)
                                        )
                                        .map((item) =>
                                            selectedBookId === item.bookId ? (
                                                <li key={item.id}>
                                                    <button
                                                        ...
                                                        className={`w-full h-fit bg-[#000]/60 backdrop-blur-[50px] hover:bg-[#32CDF0] text-white px-4 py-2 rounded-[10px] ${locale === 'km' ? 'font-krasar' : 'font-gotham'
                                                            }`}
                                                    >
                                                        {locale === 'km' ? item.nameKm : item.nameEn}
                                                    </button>
                ...
                {/* Version Popup */}
                {showVersionPopup && (
                    <div className={`fixed z-10 inset-0 w-[95%] sm:w-[90%] md:w-full mx-auto h-[60vh] flex items-center justify-center 
                ...
                {/* Chapter Popup */}
                {showChapPopup && (
                    <div className={`fixed z-10 inset-0 md:translate-x-[47%] translate-y-[10%] w-[95%] sm:w-[90%] md:w-[50%] mx-auto h-[60vh] flex items-center justify-center 
                    transition-all duration-500 ease-in-out 
