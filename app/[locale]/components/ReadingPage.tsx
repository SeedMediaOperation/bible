...
const ReadingPage = ({versions, books, chapters,params}:ReadingProps) => {
    ...
    const [selectedBookName, setSelectedBookName] = useState<string | null>('Genesis');
    const [selectedVersionName, setSelectedVersionName] = useState<string | null>(formatSplit(params!));
            ...
            <div className="w-full h-[50vh] md:h-fit bg-[rgba(30,_30,_30,_0.95)] border-[1px_solid_#575757] shadow-[0px_4px_4px_rgba(0,_0,_0,_0.25)] rounded-[40px] overflow-y-hidden">
                <div className="flex space-x-2 p-5">
                    ...
                    >
                        ...
                        {currentBook ? `${currentBook.nameEn} ${currentChapter?.nameEn}` : `Chapter ${activeChapterIndex + 1}`}
                        </span>
                    ...
                    <button
                        ...
                        <span className="text-[12px] md:text-[16px]">Change Chapters</span>
                        <span>
                            <svg width="12" height="12" viewBox="0 0 16 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M9.73205 12C8.96225 13.3333 7.03775 13.3333 6.26795 12L1.0718 3C0.301995 1.66667 1.26424 1.46309e-06 2.80384 1.32849e-06L13.1961 4.19966e-07C14.7358 2.8537e-07 15.698 1.66667 14.9282 3L9.73205 12Z" fill="#00AFD7"/>
                            </svg>
                    ...
                    {filteredBook.map((p, chapterIndex) => {
                        ...
                        return (
                            <SwiperSlide key={chapterIndex}>
                                <h1 className="text-[14px] md:text-[20px] text-center font-bold my-2">
                                    {matchedBook.nameEn} {currentChapter?.nameEn}
                                </h1>

                                <h1 className="text-[16px] md:text-[20px] font-[600] my-2 text-balance me-[10rem] mb-5">
                                    {p.titleEn}
                                </h1>

                                {p.paragraphEn.map((line, index) => {
                ...
                <div className={`fixed z-10 inset-0 w-full md:w-[50%] md:translate-x-[-47%] translate-y-[10%] mx-auto h-fit flex items-center justify-center 
                    ...
                    <div className="bg-black/80 backdrop-blur-[50px] rounded-[20px] p-4 shadow-lg w-full h-full overflo-y-hidden">
                        <div className="flex justify-between items-center">
                            {!showChapterPopup ?
                                <h2 className="text-lg font-bold mb-4 text-[#fff]">Book</h2>
                                :
                                <button onClick={() => {
                        ...
                        <div className={`w-full inline-flex justify-center`}>
                            <ul className={`w-full !h-[44vh] ${!showChapterPopup ? '!translate-0 opacity-100' : '!-translate-x-[120%] opacity-0 !w-0'} transition-all duration-500 ease-in-out space-y-2 overflow-y-auto`}>
                                ...
                                <li key={item.id}>
                                    ...
                                    className="w-full bg-[#32CDF0]/30 hover:bg-[#32CDF0] text-white px-4 py-2 rounded-[10px]"
                                    >
                                    {item.nameEn}
                                    </button>
                                </li>): 
...
transition-all duration-500 ease-in-out overflow-y-auto`}>
                                {chapters.map((item) =>
                                    ...
                                    <li key={item.id}>
                                        ...
                                        className="w-full h-full bg-[#000]/60 backdrop-blur-[50px] hover:bg-[#32CDF0] text-white px-4 py-2 rounded-[10px]"
                                        >
                                        { item.nameEn }
                                        </button>
                                    </li>
            ...
            {showVersionPopup && (
                <div className={`fixed z-10 inset-0 w-full mx-auto h-fit flex items-center justify-center 
                    ...
                    <div className="bg-black/80 backdrop-blur-[50px] rounded-[20px] p-4 shadow-lg w-full h-full overflow-hidden">
                        <div className="flex justify-between items-center">
                            {!showChapterPopup ?
                                <h2 className="text-lg font-bold mb-4 text-[#fff]">Version</h2>
                                :
                                <button onClick={() => {
                                ...
                                <li key={item.id}>
                                    <button
                                        onClick={async () => {
                                            ...
                                            await router.replace(`/${routing.defaultLocale}/${encodeURIComponent(item.slug!)}`);
                                        }}
                                    className="w-full bg-[#32CDF0]/30 hover:bg-[#32CDF0] text-white px-4 py-2 rounded-[10px]"
                                    >
                                        {item.titleEn}
            ...
            {showChapPopup && (
                <div className={`fixed z-10 inset-0 translate-x-[47%] translate-y-[50%] w-[50%] mx-auto h-fit flex items-center justify-center 
                    ...
                    <div className="bg-black/80 backdrop-blur-[50px] rounded-[20px] p-4 shadow-lg w-full h-full overflow-hidden">
                        <div className="flex justify-between items-center">
                            <h2 className="text-lg font-bold mb-4 text-[#fff]">Chapter</h2>
...
transition-all duration-500 ease-in-out overflow-y-auto`}>
        ...
        {highlightPopup && 
            <div className="fixed left-0 right-0 bottom-0 w-[340px] mx-auto h-[28vh] bg-white z-[2] rounded-t-[30px] shadow-sm drop-shadow-lg transition-all duration-[500]">
                ...
                <h1 className="text-[16px] p-5 font-bold">
