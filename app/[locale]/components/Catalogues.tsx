            ...
            <div className='w-full h-full md:h-fit flex flex-wrap justify-center gap-2 md:gap-[1rem] mt-4 overflow-y-auto overflow-x-hidden'>
                {catalogue && catalogue.map((cata, index) =>
                    <Link key={index}
                        ...
                        <h1 className={`w-[120px] mx-auto text-center text-[12px] text-[#fff] uppercase text-wrap my-2 ${locale === 'km' ? 'font-krasar':'font-gotham'}`}>{locale === 'km'? cata.name_km : cata.name_en}</h1>
                    </Link>
                )}
            </div>
        </div>
