...
import { Catalogue, CatalogueProps } from "@/types/catalogue";
...
export default function Context({ catalogue,pagination:initialPagination,onDelete, submit }: CatalogueProps) {
    ...
    const [localData, setLocalData] = useState<Catalogue[]>(catalogue);
    const [filteredData, setFilteredData] = useState<Catalogue[]>(catalogue);

    const handleOpenModal = (type: 'add' | 'edit', data?: Catalogue) => {
        setMode(type);
        setForm(data || {id:"", name_en:'', name_km:'', image:''});
                        ...
                        {loading ? (
                            ...
                            filteredData.map((item, index) => 
                                    <tr key={item.id}>
                                        ...
                                        <td className={`dropdown ${catalogue.length > 10 ? 'dropdown-top dropdown-end' : 'dropdown-end'}`}>
