...
import { Version, VersionProps } from "@/types/book";
import React, {
    useEffect,
    useRef,
    useState,
...
export default function Context({ versions,onDelete, submit,pagination:initialPagination }: VersionProps) {
    ...
    const [localData, setLocalData] = useState<Version[]>(versions);
    const [filteredData, setFilteredData] = useState<Version[]>(versions);
    const [confirmDeleteUserId, setConfirmDeleteUserId] = useState<string | null>(null);


    const handleOpenModal = (type: 'add' | 'edit', data?: Version) => {
        setMode(type);
        setForm(data || { titleEn: '', titleKm: '' });
              ...
              if (search) {
                const lowerSearch = search.toLowerCase();
                    data = data.filter((item) =>
                        item.titleEn?.toLowerCase().includes(lowerSearch.toLowerCase()) ||
                    item.titleKm?.toLowerCase().includes(lowerSearch.toLowerCase())
                );
              }
            ...
            <div className="hidden md:block">
                <table className="table w-full text-sm md:text-base">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th className="hidden sm:table-cell text-gray-100">Title English</th>
                            <th className="hidden lg:table-cell text-gray-100">Title Khmer</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                                ...
                                <tr key={version.id}>
                                    ...
                                    <td className={`dropdown ${versions.length > 10 ? 'dropdown-top dropdown-end' : 'dropdown-end'}`}>
                                        ...
                                        <ul tabIndex={0} className="dropdown-content menu bg-gray-900 rounded-box z-1 w-52 p-2 shadow-sm">
                                            ...
                                            <li className="w-full">
                                                <button
                                                    onClick={() => setConfirmDeleteUserId(version.id ?? '')}
