...
import schema from "@/app/api/vlogs/schema";
    ...
    const totalPages = Math.ceil(totalUsers / limit);
      ...
      if (page > totalPages) {
        ...
        return NextResponse.json(
            {
                data: vlog,
                message: "Successfully Getting Vlog",
                pagination,
                status: 200
            },
            {status:200}
...
export async function POST(req:NextRequest){
    try {
        ...
        const newVlog = await prisma.vlog.create({
            data:{
        ...
        if(!newVlog){
            return NextResponse.json({
                message: "Failed to created vlog"
            },{status:500})
        }
        ...
        const message = error instanceof Error ? error.message : String(error);
        return new NextResponse("Error in fetching Chapter: " + message, {status:500})
    }
}